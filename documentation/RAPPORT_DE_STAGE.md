...
│  ┌─────────────┐                                                             │
│  │             │                                                             │
│  │    Admin    │──── Gérer les employés (CRUD)                              │
│  │             │──── Gérer les clients (CRUD)                               │
│  │             │──── Gérer les projets (CRUD)                               │
│  │             │──── Assigner employés aux projets                          │
...
│  ┌─────────────┐                                                             │
│  │             │                                                             │
│  │   Employé   │──── Consulter ses projets assignés                         │
│  │             │──── Gérer ses tickets assignés                             │
│  │             │──── Mettre à jour progression                              │
│  │             │──── Télécharger/Consulter fichiers                         │
│  │             │──── Modifier son profil                                    │
│  └─────────────┘                                                             │
│                                                                              │
│  ┌─────────────┐                                                             │
│  │             │                                                             │
│  │   Client    │──── Consulter ses projets                                  │
│  │             │──── Créer des tickets                                      │
│  │             │──── Suivre ses tickets                                     │
│  │             │──── Télécharger fichiers projet                            │
│  │             │──── Modifier son profil                                    │
│  └─────────────┘                                                             │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
│ - email          │1     1│ - phone          │       │ - company_name   │
│ - password       │───────│ - position       │       │ - contact_name   │
│ - role           │       │ - department     │       │ - phone          │
│ - created_at     │       │ - skills (JSON)  │       │ - address        │
└──────────────────┘       │ - hire_date      │       │ - logo           │
                           └──────────────────┘       └──────────────────┘
                                    │                         │
                                    │ N                       │ 1
                    ...
                    │ N             │               │ N
           ┌────────┴─────┐  ┌─────┴──────┐  ┌─────┴──────┐
           │    Ticket    │  │    File    │  │Notification│
           ├──────────────┤  ├────────────┤  ├────────────┤
           │ - id         │  │ - id       │  │ - id       │
           │ - project_id │  │ - project_id│ │ - user_id  │
           │ - client_id  │  │ - uploaded_by│ │ - type    │
           │ - employee_id│  │ - file_name │ │ - title    │
           │ - title      │  │ - file_path │ │ - message  │
           │ - description│  │ - file_type │ │ - priority │
           │ - priority   │  │ - file_size │ │ - is_read  │
           │ - status     │  └────────────┘  └────────────┘
           └──────────────┘
                    │ 1
                    │
...
La base de données **promptis_manager** comprend les tables suivantes :

| Table | Description | Relations |
|-------|-------------|-----------|
| `users` | Utilisateurs du système | Base pour tous les rôles |
| `employees` | Détails des employés | 1-1 avec users |
| `clients` | Détails des clients | 1-1 avec users |
| `projects` | Projets de l'entreprise | N-1 avec clients |
| `project_employee` | Affectations (pivot) | N-N entre projects et employees |
| `tickets` | Tickets de support | N-1 avec projects, clients |
| `ticket_comments` | Commentaires tickets | N-1 avec tickets |
| `files` | Fichiers uploadés | N-1 avec projects |
| `notifications` | Notifications système | N-1 avec users |
...
> **Note :** Insérer ici les captures d'écran de l'application :
> 
> 1. **Page d'accueil (Landing Page)** - Présentation de Promptis Manager
> 2. **Page de connexion** - Formulaire de login
> 3. **Dashboard Admin** - Vue d'ensemble avec statistiques
> 4. **Liste des employés** - Tableau avec actions
> 5. **Modal création projet** - Formulaire avec sélection d'employés
> 6. **Liste des tickets** - Avec badges de priorité
> 7. **Système de notifications** - Dropdown avec alertes
> 8. **Page de profil** - Modification des informations

*(Les captures d'écran doivent être prises de l'application en fonctionnement et insérées dans le document final)*
